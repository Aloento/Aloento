...
是一种常用的无监督学习算法，能够自动将数据分为 K 个簇。

1. 你需要手动指定 K 的值，即数据分成多少个类别
2. 随机初始化 K 个中心点
3. 计算每个数据点到所有 K 个中心点的距离
4. 将数据点分配到距离最近的中心点
5. 计算每个簇所有数据点的平均值
6. 将平均值作为新的中心点
7. 重复 3-6 步骤，直到中心点变化很小或达到最大迭代次数

- Convergence：K-Means 一定会在有限次迭代后收敛
- 将数据点分配到最近的中心点：$O(n \cdot k)$
- 更新中心点：$O(n)$
...
1. 运行 K-Means，计算 总平方误差 (Sum of Squared Error, SSE)
2. 画出 SSE 随着 K 值的变化图
3. 找到拐点，即 SSE 开始下降的速度变慢的点
4. 拐点的值就是最佳的 K 值
