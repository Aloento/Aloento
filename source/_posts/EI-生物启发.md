...
由于进化需要经历几万代的试错，在真实的物理世界中是不可能完成的（电机会烧毁，金属会磨损）。所以进化100%都是在计算机的物理模拟器（Simulator）中进行的。

模拟器里的物理法则是不完美的。模拟器里的摩擦力、空气阻力、电机响应延迟与现实世界存在微小差异。
一个在模拟器里进化出完美步态的虚拟机器人，一旦将它的“基因”下载到现实世界的物理机器人身上，往往会直接摔倒抽搐。这就是“现实鸿沟”。

为了跨越这个鸿沟，理论界提出了域随机化（Domain Randomization）技术。在虚拟进化时，工程师会故意给模拟器加入随机的“噪音”：每一代机器人的地面摩擦力一会儿大一会儿小，重力甚至也会微调。只有在这种极端混乱的虚拟环境中进化出“抗干扰能力极强”的步态，才能最终成功迁移到现实世界。

通过形态协同进化、适应度地形寻优、严谨的数学打分机制以及跨越现实鸿沟的技术，进化运动学让机器人摆脱了人类有限的想象力，真正依靠算力“长”出了行走的能力。
